...
*Last updated: Session 8 (2026-09-08). Large client-feature session plus several server-side fixes. Every server change is deployed to EC2 (backup filenames noted below), each with `node --check` + `curl /health` + a targeted live test. Every client change is **JS-only, parse-checked (`@babel/core`) + mojibake-clean, but NOT yet run on a device/emulator** — a full device pass over all of the below is outstanding. All work landed on `main` (commits `250d1cc`, `2873b8c`, `403bd18`, `a4e55ad`, `fa468ad`, `19be509`).*
...
- **User profile modal + favourites** — new `src/components/UserProfileModal.js` (bottom sheet from the ChatScreen header — 1:1 and group layouts), `src/components/FavouriteStar.js` (RN `Animated` star + particle burst), `src/screens/ContactNotificationSettings.js` (per-contact notification prefs), `src/utils/favourites.js` + `src/utils/contactPrefs.js` (`wave_favourites` / `wave_mute_{id}` / `wave_notifications_{id}` in AsyncStorage), favourites strip in `ChatListScreen.js`, mute state lifted into `ChatScreen.js`. Server: `routes/conversations.js` `GET /` + `POST /start` now return `profilePicture` on `with` / `members` (`routes/conversations.js.bak-1788862250`). See "User profile modal".
...
- **Status confirmed (no code change)** — ringtone playback verified working on a real device (plays + loops on the callee, clean handoff to call audio on accept, stops on reject / remote-cancel / 60s timeout); the 60s server-authoritative call-timeout ring behaviour confirmed 2-device alongside it.
...
- **NEXT TASK** — build + install a dev client (`npx expo run:android` over USB) and run: (1) the full push-notification 2-device checklist in Section G, (2) the call-glare 2-device test, (3) the account deactivate/delete 2-device test, (4) a device pass over every Session-8 client UI above (incl. the new change-email/phone flows).
