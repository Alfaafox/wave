...
- Push notifications — BUILT & DEPLOYED (Session 6), FCM wired + backgrounded-message-push fix (Session 7), see Section G. `google-services.json` wired into `android/` via prebuild; FCM V1 key on Expo; direct send verified (ticket+receipt `ok`). Session 7 also fixed message push never firing for backgrounded-but-connected clients (`foregroundUsers` + `presence:*` events, server deployed). **Remaining: build + install a dev client with the Session-7 changes (`npx expo run:android` / EAS dev) and run the 2-device test in Section G** — the client currently on devices predates all of this.
- Rate limiting on login/signup — PARTIAL (Session 5-6): the in-memory `rateLimit(key, max, windowMs)` now lives in `rateLimiter.js` (shared by `routes/auth.js` and `routes/notifications.js`). Covers `/auth/login`, `/auth/forgot-password`, `/auth/reset-password`, `/auth/verify-otp`, `/notifications/token` (20/10min/user). Not yet on `/auth/signup`. In-memory = per-process only; move to Redis if scaled past one pm2 instance.
...
**Still outstanding — the real 2-device test on a build that has the Session-7 foreground-presence changes:** (a) `getExpoPushTokenAsync` returns a token and `POST /notifications/token` stores it, (b) **app backgrounded** (not just fully closed) + message from another device → "New message" push after the 8s grace, (c) app foregrounded → NO push, message arrives live, (d) `call:invite` → "Incoming call from X" wakes a backgrounded app AND the call still connects, (e) missed call → "Missed call from X", (f) tap-routing (message/call/missedCall) lands on the right screen incl. cold start, (g) logout de-registers the token. Needs `npx expo run:android` or `eas build --profile development --platform android` first.
...
- **`rateLimiter.js`** (new): the in-memory limiter extracted from `routes/auth.js` (which now `require`s it).
