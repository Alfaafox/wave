...
- **Audio-output routing** lives in the `modules/expo-call-audio` native module (see below). `CallScreen.js` has one route button (always visible on audio **and** video calls) that seeds from `getAvailableRoutes()` / `getAudioRoute()`, tracks the `onAudioRouteChanged` event (re-reading the available-routes list each time — a headset connect/disconnect changes both), and is **smart**: with 2 routes (earpiece + speaker) a tap toggles directly; with 3 routes (a Bluetooth headset connected) a tap opens **`src/components/AudioOutputSheet.js`** — an RN-core `Modal` + `Animated` + `PanResponder` bottom sheet (same pattern as `UserProfileModal` / `ImageViewerModal`, no gesture-handler) listing only the connected routes with an Ionicons `checkmark` on the active one; picking a row calls `setAudioRoute` and closes. The sheet auto-closes if the headset drops while it's open. Icons: `bluetooth` (blue `#4DA3FF`) / `ear-outline` / `volume-high-outline`.

### Custom native module: `modules/expo-call-audio`

A local Expo Module (Android Kotlin / iOS Swift / web no-op) that forces the OS into in-call audio mode (`AudioManager.MODE_IN_COMMUNICATION` + audio focus on Android) and drives audio-output routing. It exists because `react-native-incall-manager` and `expo-audio`'s `setAudioModeAsync` do **not** affect the audio subsystem WebRTC actually uses on the New Architecture. `callManager.js` calls `startCallAudio(isVideo)` when local media is acquired and `stopCallAudio()` in `cleanup()`. Changing this module requires a native rebuild.

**Android routing (`ExpoCallAudioModule.kt`, Bluetooth support added ~Session 14 "Bluetooth SCO audio routing", `isVideo` fallback fixed the session after Session 13):**
- Routes are the strings `"bluetooth" | "earpiece" | "speaker"`, shared verbatim with JS. API 31+ uses `AudioManager.setCommunicationDevice()` / `clearCommunicationDevice()`; API <31 falls back to `startBluetoothSco()` / `setBluetoothScoOn()` / `isSpeakerphoneOn`. "bluetooth" means a **call-capable** BT device only — `TYPE_BLUETOOTH_SCO` or (API 31+) `TYPE_BLE_HEADSET`; A2DP is media-only and is **not** a call route.
- `startCallAudio(isVideo)` runs Bluetooth detection **first for both audio and video calls** — a connected SCO/LE headset always wins. Only with no headset does the call type decide: video → speaker, audio → earpiece. Emits `onAudioRouteChanged`.
- `isVideoCall` is stored as a `@Volatile` module field (set in `startCallAudio`, cleared in `stopCallAudio`). An `AudioDeviceCallback` registered for the call auto-switches to a headset that connects mid-call and, when one **disconnects**, falls back the way a fresh call of that type would start — **video → speaker, audio → earpiece** (not always earpiece).
- JS surface: `getAudioRoute()` (current route, reads `communicationDevice` on API 31+), `getAvailableRoutes()` (`["bluetooth","earpiece","speaker"]` when a headset is connected, else `["earpiece","speaker"]` — bluetooth first), `setAudioRoute(route)` (unknown / unavailable → earpiece), `setSpeakerphoneOn(bool)` (kept, routed through the same path), `addListener('onAudioRouteChanged', …)`.
- Also hosts `setSecureScreen(enabled)` (Activity `FLAG_SECURE`) — used by the view-once photo viewer, unrelated to calls; this is just the only local native module.
...
### Shared Media screen (Session 12)

`src/screens/SharedMediaScreen.js` — "Media, Links & Docs", opened from the row of that name in `UserProfileModal` (which previously `Alert`ed "Coming soon"). Rendered by **`ChatScreen` as an absolute-fill overlay** (`styles.notifSettingsOverlay`, `sharedMediaOpen` state) — the exact pattern `ContactNotificationSettings` uses; **no new `App.js` screen state** (checked App.js: sub-screens off the profile modal are ChatScreen overlays, not screen-machine entries). `UserProfileModal` gets an `onOpenSharedMedia` prop; back returns to the profile modal.

**Backend** — `GET /conversations/:id/media?type=images|links&limit=30&offset=0` (`routes/conversations.js`, `.bak-1788934408`). Checks in order: membership → 403; private-chat 7-day `created_at` window; `type` must be `images`/`links` → 400; `rateLimit('media:<uid>', 60, 60_000)` → 429. `images`: `message_type='image'`, `deleted_for_everyone=0`, `ORDER BY m.created_at DESC, m.id DESC` (id tiebreak added so same-second sends stay newest-first) → `results:[{id,content,created_at,user_id,sender_name}]` + `total`. `links`: `message_type='text' AND content LIKE '%http%'`, paginated by **message**, then each row's URLs extracted with `/(https?:\/\/[^\s]+)/g` and **flattened to one result per URL** (`{id,url,created_at,user_id,sender_name}`); `total`/`hasMore` track the message pagination. `sender_name` is `COALESCE(u.name, m.username)` (never null). `limit` capped at 30. Scoped by `m.conversation_id` after the membership check — nothing from other conversations. 21-assertion e2e green.

**Client** (`api.js` `getSharedMedia`). Three pill tabs (local `activeTab` state, no lib). Each tab caches its results in state and **does not refetch on tab switch once loaded**; base64 image strings mean strict 30/page pagination and only what's been scrolled to is held.
- **Media**: one `FlatList` whose `data` is a flat `[{type:'header',title} | {type:'row',images:[<=3]}]` list — this is how it carries **month/year section headers AND a correct fixed-size `getItemLayout`** (a real `numColumns={3}` grid can't host full-width headers, and `getItemLayout` on `SectionList` has fragile flattened-index semantics with no helper lib available — deliberate deviation from the brief's literal "numColumns=3 / SectionList"). `removeClippedSubviews`, `maxToRenderPerBatch={3}` / `initialNumToRender={4}` / `windowSize={5}` (per **row-item**, ~9 / ~12 images — the brief's per-image figures), `onEndReached` (threshold 0.3) → next page. Cell = `screenWidth/3` square, `resizeMode="cover"`. Tap → the existing `ImageViewerModal` (`key={uri}` to remount per image; `onSave` wired to ChatScreen's `saveImage`, now `useCallback`-stable). Long-press → `Alert` "Save to Gallery" (`saveImage`) / "Share" (`Share.share({message:uri})` — RN core; base64 sharing is limited, http(s) GIF/sticker URLs share fine) / Cancel. Empty: `image-outline` + "No photos or videos yet".
- **Links**: flat `FlatList`, each row `globe-outline` + domain (`new URL(...).hostname` with a regex fallback) + full URL (grey) + short date; tap → `Linking.openURL(url).catch(Alert)`. Empty: `link-outline` + "No links shared yet".
...
- **`/android` and `/ios` are gitignored** (`app.json` treats them as prebuild output) but `android/` is currently checked out locally as a prebuilt project. Native/config-plugin changes require re-running prebuild or a fresh EAS build; they won't hot-reload. Any hand edit to `AndroidManifest.xml` / gradle files is lost on the next `expo prebuild` — persist it via `app.json` or a config plugin in `plugins/` (e.g. `plugins/withFullScreenCall.js`, Session 15).
...
- Bluetooth audio routing — DONE (native module + `CallScreen` smart button / `AudioOutputSheet`, see "Calls (WebRTC)" and the `expo-call-audio` section). Call-capable BT (SCO / LE headset) auto-routes on call start and mid-call connect/disconnect, with a WhatsApp/Signal-style bottom-sheet picker. **Not device-tested with a real headset.** Still open: A2DP-only devices aren't offered (correct — they can't carry the call mic), and there's no per-device naming (it's just "Bluetooth", not the headset's name).
...
- **Full-screen incoming call (Session 15)** — `android/app/src/main/AndroidManifest.xml` needs `<uses-permission android:name="android.permission.USE_FULL_SCREEN_INTENT"/>` + `android:showWhenLocked="true"` / `android:turnScreenOn="true"` on `.MainActivity`. Since `/android` is gitignored prebuild output, these are **persisted through `expo prebuild`**: the permission via `app.json` → `expo.android.permissions`, and the two activity attributes via **`plugins/withFullScreenCall.js`** — a tiny `withAndroidManifest` config plugin (`@expo/config-plugins`, already installed; no new dep) registered last in `app.json` → `expo.plugins`. It calls `AndroidConfig.Manifest.getMainActivityOrThrow` and sets the two attrs (idempotent). Verified: remove the attrs, `npx expo prebuild -p android --no-clean`, they reappear. Commits `38336da` + `6cc40f5` on `main`. **Not yet device-tested** — needs `npx expo run:android` (native manifest change) + a locked-device incoming-call test.
- **`src/utils/notifications.js`** (new): device_id = `Crypto.randomUUID()` in AsyncStorage (`wave:deviceId`); `wave:notifPermissionAsked` flag so we ask exactly once; Android channels `messages` (HIGH) / `calls` (MAX); `registerDeviceForPush` / `unregisterDeviceForPush`; foreground handler returns `shouldShowBanner:false` (App shows its own banner). All fail-soft. **The `calls` channel also declares `fullScreenIntent: true` (Session 15)** so a ringing-call push raises the app over the lock screen instead of just a heads-up; paired with the manifest changes below.
...
*Last updated: Session 17 (2026-09-18).*

*Session 17 — **Design system pass: `colors.screenBackground`, `WaveButton`, toggle colors, typography scale** (see "Design System" above for the full breakdown). A design refresh was requested that, taken literally, would have wholesale-replaced `theme.js` and dropped the `shadow` export — which 17 files (including `ChatScreen.js`/`ChatListScreen.js`) spread as `...shadow.md` at module-load time, so that would have crashed the app on launch; confirmed with the user to go additive-only instead, touching nothing outside what was asked. New: `src/components/WaveButton.js` (variant/size/disabled/loading/fullWidth/icon, press-scale animation, documented Ionicons-in-Animated.View crash risk + fallback in a comment); new additive `theme.js` tokens (`colors.primary*`/`screenBackground`/`surfaceElevated`/`destructive*`/`toggleActive`/`toggleInactive`/etc., new `typography` scale, new separate `radius` export, new default export) with every existing key/value left untouched. `colors.screenBackground` (off-white, dedicated - NOT a repoint of `colors.background`, which stays white) applied to `SettingsScreen.js`, `ProfileScreen.js`, `GroupInfoScreen.js`, and via a new optional `backgroundColor` prop on `SettingsSubScreenLayout.js` to `AccountSettingsScreen.js`/`PrivacySettingsScreen.js`/`NotificationsSettingsScreen.js`/`ChatsSettingsScreen.js` (the prop defaults to the old background, so `AppearanceSettingsScreen.js`/`ContactNotificationSettings.js`/`InviteFriendScreen.js`, which share the same layout but were never asked for, are unaffected). All 5 `Switch`es in Privacy/Notifications/Chats settings got `trackColor`/`thumbColor`/`ios_backgroundColor`. `WaveButton` wired into the Log Out buttons (Settings/Profile), Leave Group (GroupInfo), and Invite a Friend; `StatusCreatorScreen.js`'s Post button became an inline `Pressable` pill instead (a full-screen camera view - `WaveButton`'s `fullWidth` would break that header row). `GroupInfoScreen.js`'s invite-link card now shows only the `https://` URL, not `wave://`. Every touched file re-parsed clean with `babel-preset-expo`. `ChatScreen.js` (4255 lines, not edited by this task) was not read in full - verified by targeted grep instead (its `Switch`/`colors.background` usage) given zero planned edits there; noted here rather than silently skipped. Client JS-only, **not device-tested**.*

*Session 16 — **ConfirmModal migration completed for the flagged Alert.alert calls** (UI consistency, no functional/behavioral change). The `src/components/ConfirmModal.js` + `EmptyState.js` rollout to most screens (Calls/ChatList/Chat/ChatsSettings/GroupInfo/PrivacySettings/Profile/ScheduledMessages/StatusViewer/Updates) plus the "Alert.alert() — remaining known calls" follow-up list were already in the working tree, uncommitted, from an earlier unrecorded pass. This session cleared that follow-up list: `AccountSettingsScreen.js` Deactivate Account confirm; `StarredMessagesScreen.js` Unstar message confirm; `PinnedMessagesModal.js` Unpin message confirm; `UserProfileModal.js` Private Chat toggle + Leave Group confirms (share one `confirmDialog` state object + a single `ConfirmModal` instance, since the two dialogs are mutually exclusive on that sheet — `handleBlock`'s block/unblock stayed on `Alert.alert`, it was never flagged). `ChatListScreen.js`'s Delete-chat confirm moved to `ConfirmModal`, but its row long-press menu (Archive / Delete chat / Cancel) could not — `ConfirmModal` only expresses a single confirm/cancel pair, not a 3-option action sheet — so that menu is now a small custom bottom-sheet `Modal` reusing the file's existing `composeSheet` styling; picking "Delete chat" from it opens the `ConfirmModal`. `ArchivedChatsScreen.js` was flagged as "Delete chat confirm" but never actually had one — its only dialog was an Unarchive/Cancel row long-press menu — so it got the same bottom-sheet treatment for visual consistency with ChatListScreen; no `ConfirmModal` was needed since unarchiving isn't destructive (flagged-list wording corrected in the section below). All six files re-parsed clean with `babel-preset-expo` and passed the mojibake grep. Client JS-only, **not device-tested**. Uncommitted at time of writing (same branch as the pre-existing ConfirmModal rollout).*

*Session 15 — **Full-screen incoming-call notification** (iOS-parity groundwork / Phase 2). When a `call:invite` push lands on a locked or backgrounded Android device the app should raise the incoming-call UI over the lock screen, not just a heads-up notification. Three pieces: (1) `src/utils/notifications.js` — `fullScreenIntent: true` on the `calls` notification channel (already `AndroidImportance.MAX`); (2) `app.json` — `android.permission.USE_FULL_SCREEN_INTENT` added to `expo.android.permissions`; (3) **`plugins/withFullScreenCall.js`** (new) — a `withAndroidManifest` config plugin (`@expo/config-plugins` 57.0.9, already installed — no new dep) that sets `android:showWhenLocked="true"` + `android:turnScreenOn="true"` on `.MainActivity`, registered last in `app.json` → `expo.plugins`. This makes all three survive `expo prebuild` (which regenerates the gitignored `android/`). Verified: `npx expo prebuild -p android --no-clean` applies them; after deleting the two activity attrs by hand and re-running, the plugin puts them back. `babel`/JSON parse + mojibake scan clean. Commits `38336da` (manifest + channel) + `6cc40f5` (plugin + app.json) on `main`. **Not device-tested** — needs `npx expo run:android` (native manifest change) then a real locked-device incoming-call test; also note Android 14+ restricts `USE_FULL_SCREEN_INTENT` to calling/alarm apps (granted by default for those categories, revocable in Settings).*

*Session 14 — **Bluetooth call audio: `isVideo`-aware fallback + audio-output bottom sheet** (Phase 2). Native (`modules/expo-call-audio/.../ExpoCallAudioModule.kt`): added a `@Volatile isVideoCall` module field (set in `startCallAudio`, cleared in `stopCallAudio`); `AudioDeviceCallback.onAudioDevicesRemoved` now falls back **video → speaker / audio → earpiece** when a BT headset drops mid-call (was always earpiece). `startCallAudio` already ran BT detection for both audio and video calls (Session ~14 "Bluetooth SCO audio routing", commit `5078117`) — comments clarified; `getAudioRoute()` / `getAvailableRoutes()` verified, unchanged. `CallScreen.js`: `cycleAudioRoute` → `applyAudioRoute` + smart `handleAudioButtonPress` (2 routes = direct toggle, 3 routes = open the sheet); the route button is now **always visible on video calls too**; `onAudioRouteChanged` re-reads `getAvailableRoutes()`; `controls` row got `flexWrap` so the extra button can't push a control off-screen; sheet auto-closes if the headset drops. New **`src/components/AudioOutputSheet.js`** — RN-core `Modal` + `Animated` + `PanResponder` bottom sheet (drag handle, tap-outside dismiss, spring-in / timing-out, dark call-screen theme, one row per connected route + Ionicons `checkmark` on the active one), same pattern as `UserProfileModal` / `ImageViewerModal`, no new deps. Verified: `./gradlew :expo-call-audio:compileDebugKotlin` OK, `babel-preset-expo` parse OK, `npx expo export --platform android` clean 4.8 MB bundle, mojibake scan clean. **Needs `npx expo run:android` (native rebuild for the Kotlin change) + a real-headset device test.** Commit `ea8c4c6` on `main`.*
...
## UI Components (shared, use everywhere — do not recreate)

### src/components/ConfirmModal.js
Replaces all Alert.alert() confirmation dialogs. Props: visible, variant ('destructive'|'warning'|'confirm'|'info'), title, body, confirmLabel, cancelLabel, onConfirm, onCancel.
Usage pattern — one modal state object per screen:
  const [modal, setModal] = useState({ visible: false });
  const showModal = (config) => setModal({ visible: true, ...config });
  const hideModal = () => setModal({ visible: false });
  <ConfirmModal visible={modal.visible} variant={modal.variant} title={modal.title} body={modal.body} confirmLabel={modal.confirmLabel} cancelLabel={modal.cancelLabel} onConfirm={() => { hideModal(); modal.onConfirm?.(); }} onCancel={hideModal} />
Never use Alert.alert() for confirmations or destructive actions. Always use ConfirmModal.

### src/components/EmptyState.js
Shared empty state for screens with no content. Props: icon ('chat'|'phone'|'status'|'search'|'clock'), title, body.
Show when: data array is empty AND loading is false. Never show on a blank/loading screen.

### src/components/WaveButton.js
Shared button — use this for new buttons instead of a raw `TouchableOpacity`/`Pressable`. Props: `label`, `onPress`, `variant` ('primary'|'secondary'|'destructive'|'ghost', default 'primary'), `size` ('md'|'sm', default 'md'), `disabled`, `loading`, `fullWidth`, `icon` (Ionicons name, optional). `fullWidth` stretches to the parent's width — wrap it in a `View` if you need margins, since the component takes no style/margin prop itself. Pill shape (`radius.pill`) for primary/destructive, `radius.md`/`radius.sm` for secondary/ghost depending on `size`. Colors come from the new `colors.primary*`/`colors.destructive*` tokens in `theme.js` (see "Design System" below) — never hardcode. Known risk documented in-file: Ionicons renders inside the press-scale `Animated.View`, a known New Architecture + Hermes crash pattern; the documented fallback (drop the scale animation, use opacity instead) is written in a comment in the file — flip to it if this ever crashes on a real device.

## Design System (2026-09-18 pass — additive only)
A design refresh was requested (off-white screen backgrounds, a shared button component, consistent toggle colors, a typography scale). `theme.js` already had 60+ dependents on its existing key names — `radii` (30 files), `colors.textPrimary`/`textMuted`/`textOnAccent`, `colors.headerBackground`/`headerBorder`, and **`shadow`, which 17 files spread as `...shadow.md`/`...shadow.sm` inside `StyleSheet.create()` at module-load time** (including `ChatScreen.js` and `ChatListScreen.js` themselves) — so replacing the file wholesale, or repointing `colors.background`/`colors.surface` to new values, would have reskinned or crashed screens nothing here was meant to touch. The resolution (confirmed with the user): **additive only** — nothing existing was renamed, removed, or given a new value.

- **New tokens in `colors`**: `primary`/`primaryDark`/`primaryLight` (same blue as `accent`), `screenBackground` (`#F2F3F5` — the new off-white, a *dedicated* token, NOT a repoint of `colors.background` which stays `#FFFFFF` and is still what ChatScreen/ChatListScreen/Login/Signup and 20+ other files read), `surfaceElevated` (`#FFFFFF`), `text`, `textTertiary`, `destructive`/`destructiveDark`/`destructiveLight`, `success`, `toggleActive`/`toggleInactive`, `sentBubble`/`receivedBubble`/`sentText`/`receivedText`. Existing `textSecondary`/`border`/`divider`/`warning` were NOT redefined even though the original ask specified slightly different hex values for them — they already existed with different values depended on elsewhere.
- **New `typography` keys**: `title`/`heading`/`body`/`bodyMedium`/`secondary`/`caption` (18/700, 17/600, 15/400, 15/500, 13/400, 11/400) — added alongside the existing `headerTitle`/`rowName`/`rowPreview`/`timestamp`/`bubbleText` scale, which is untouched.
- **New `radius` export** (singular, sm/md/lg/pill/card) — deliberately separate from the existing `radii` export (plural, different values, 30 dependents). New code (`WaveButton.js`) uses `radius`; nothing was migrated off `radii`.
- **New default export** — `export default { colors, typography, spacing, radii, radius, shadow }`. Every existing import in the app is a named import, so this is additive.
- **`colors.screenBackground` is applied to**: `SettingsScreen.js`, `ProfileScreen.js`, `GroupInfoScreen.js` (own `container` style), and via a new optional `backgroundColor` prop on `SettingsSubScreenLayout.js` (default unchanged) to `AccountSettingsScreen.js`, `PrivacySettingsScreen.js`, `NotificationsSettingsScreen.js`, `ChatsSettingsScreen.js`. **Not** applied to `AppearanceSettingsScreen.js`, `ContactNotificationSettings.js`, or `InviteFriendScreen.js` — they share the same layout component but were never asked for, so the prop defaults to the old white background for them. Card/surface backgrounds inside all of these are untouched.
- **Toggles**: every `Switch` in `PrivacySettingsScreen.js` (3), `NotificationsSettingsScreen.js` (1), `ChatsSettingsScreen.js` (1) now has `trackColor={{ false: colors.toggleInactive, true: colors.toggleActive }}` + `thumbColor={colors.surfaceElevated}` + `ios_backgroundColor={colors.toggleInactive}`. Other `Switch`es in the app (e.g. `GroupInfoScreen.js`'s slow-mode toggle) were left alone — not in scope.
- **Buttons**: new `src/components/WaveButton.js` (see above). Wired into `SettingsScreen.js` + `ProfileScreen.js` (Log Out, `variant="destructive"`), `GroupInfoScreen.js` (Leave group, `icon="log-out-outline"`), `InviteFriendScreen.js` (Invite a Friend, `variant="primary"`, `icon="share-social-outline"`). `StatusCreatorScreen.js`'s Post button was deliberately NOT converted to `WaveButton` (full-bleed camera view, `fullWidth` would break the header-row layout) — it's an inline `Pressable` pill (`colors.primary`, `paddingHorizontal:20/paddingVertical:8/borderRadius:20`) instead.
- **Group invite link**: `GroupInfoScreen.js` now shows only the `https://waveitchat.com/join/...` link; the `wave://` deep link is no longer rendered in any `Text` (it wasn't used for anything besides display — `handleCopyInvite` already only ever copied the https link).
- **Rule going forward**: never hardcode a hex color in new code — pull from `theme.js`. When a new screen wants the off-white look, use `colors.screenBackground`, not `colors.background`.

## Alert.alert() migration status
The previously-flagged confirm/destructive dialogs are now migrated to `ConfirmModal`:
- AccountSettingsScreen.js — Deactivate Account confirm
- ChatListScreen.js — Delete chat confirm now `ConfirmModal`; the row long-press menu (Archive / Delete chat) is now a custom bottom-sheet `Modal` (matching the existing `composeSheet` pattern), not an `Alert.alert` action sheet — `ConfirmModal` only supports a single confirm/cancel pair, so a 3-option menu can't be expressed as one.
- ArchivedChatsScreen.js — this file never actually had a "Delete chat" dialog (only an Unarchive/Cancel row long-press menu); that menu is now the same custom bottom-sheet pattern as ChatListScreen's, for consistency. No `ConfirmModal` needed here since Unarchive isn't destructive.
- StarredMessagesScreen.js — Unstar message confirm
- PinnedMessagesModal.js — Unpin message confirm
- UserProfileModal.js — Private Chat toggle confirm and Leave Group confirm now share one generic `confirmDialog` state object (`showConfirm`/`hideConfirm`) rendered as a single `ConfirmModal` instance, since both are mutually exclusive dialogs on the same sheet. `handleBlock`'s block/unblock confirm was intentionally left on `Alert.alert` — it was never in the flagged list.
All other remaining Alert.alert() calls in the codebase are validation/error/permission alerts — these are acceptable and do not need migration.
