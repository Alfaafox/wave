...
*Last updated: Session 8 (2026-09-08) — (i) Online/offline presence: `server.js` `presence:online`/`presence:offline`/`presence:sync` scoped to conversation peers' `user:<id>` rooms (replaces global `presence` broadcast), respects `last_seen_visible`; `routes/conversations.js` nulls hidden `last_seen`. Deployed (`server.js.bak-1788866830`, `conversations.js.bak-1788866830`), node --check + health OK, 2-client + privacy tests pass. Client: `presenceMap` lifted to `App.js`, `ChatListScreen` green dot restyled (10px `#4CAF50`), `ChatScreen` live header "online"/"last seen". Client parse-checked, not device-tested. (h) 3-state typing indicator: new `TypingIndicator.js` (typing = 3 bouncing dots / paused = 1 breathing dot / gone), `ChatScreen.js` sender+receiver logic with throttle + safety timers, `server.js` `typing:start`/`typing:pause`/`typing:stop` relay handlers replacing the old `typing` event (deployed, `server.js.bak-1788866049`, node --check + health OK, 2-client relay + membership check verified). Client parse-checked, not device-tested. (g) User profile modal + favourites: new `UserProfileModal.js` (bottom sheet from ChatScreen header, 1:1 + group), `FavouriteStar.js` (RN Animated star + particle burst), `ContactNotificationSettings.js` (per-contact notif prefs), `src/utils/favourites.js` + `src/utils/contactPrefs.js` (AsyncStorage `wave_favourites` / `wave_mute_{id}` / `wave_notifications_{id}`), favourites strip in `ChatListScreen.js`, mute lifted into `ChatScreen.js`. Server: `routes/conversations.js` `GET /` + `POST /start` now return `profilePicture` on `with` / `members` (deployed, `routes/conversations.js.bak-1788862250`, node --check + health OK, verified `with.profilePicture` present in live response). Client parse-checked, not device-tested. (f) ProfileScreen: avatar tap → full-screen `ImageViewerModal` (gated its save button on `onSave`); "Change profile picture" button → gallery-only pick + `allowsEditing` crop + `quality: 1` full-size save (was `quality: 0.4`); ~7 MB client guard. Client-only, no new deps (expo-image-picker already does crop). (e) Email: case-insensitive accounts (server stores + looks up lower-cased in `routes/auth.js` signup/resend/`findUserByIdentifier` + `routes/users.js` `PUT /me`; deployed, `routes/auth.js.bak-1788859424` / `routes/users.js.bak-1788859488`; verified live — uppercase signup stored lowercase, mixed-case login resolves) + client-side email-format regex on Login/Signup screens with inline error (`AuthField` `error` prop, `isValidEmail` in `AuthUI.js`). (d) GIPHY attribution: "Powered by GIPHY" badge added to `MediaPickerSheet.js` (`GiphyAttribution` inside `MediaGrid`, shows on GIF + Sticker tabs only, all states). Client-only. (c) Rate limiting: `express-rate-limit` added to `server.js` on `/auth/login` + `/auth/signup` (10/15min/IP, per-path bucket); redundant hand-rolled login limiter removed from `routes/auth.js`. Deployed to EC2 (`server.js.bak-1788858324`, `routes/auth.js.bak-1788858324`), `pm2 restart` done, health OK, verified live (req 11 → 429). (b) Call glare handling built (Phase 2 / Section E): `call_signaling.js` `call:invite` acks `{ glare: true }` for crossing invites (server backup `call_signaling.js.bak-1788857358`, `node --check` OK; **pm2 restart still pending**); `src/utils/callManager.js` + `App.js` handle it with a silent teardown of the losing outgoing call, and `src/screens/CallScreen.js` now always runs `callManager.cleanup()` on unmount. Needs a 2-device test. (a) Status consolidation, no code changes. Confirmed complete: push notifications (built + deployed, FCM wired, server e2e verified — Sessions 6/7); login/signup/forgot-password redesign with Wave Chat branding (Session 5); account deactivate + hard delete (built + deployed, Section C); call ring timeout 60s (server-authoritative, deployed); **ringtone playback — now confirmed working on a real device** (plays + loops on callee, clean handoff to call audio on accept, stops on reject / remote cancel / 60s timeout), and the 60s call-timeout ring behaviour confirmed 2-device alongside it. NEXT TASK: on-device push-notification test over USB — connect the phone, `npx expo run:android` to build + install a dev client with the Session-7 presence changes, then run the full 2-device checklist in Section G.*
